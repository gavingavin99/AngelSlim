...
class SpinQuant(TransformBase):
    ...
    def _meta_fuse(self, name, linear, rotation, fuse_input=False):
        ...
        weight = linear.weight.data.to(device=DEVICE, dtype=torch.float32)
        ...
        if fuse_input:
            ...
            if hasattr(linear, "bias") and linear.bias is not None:
                bias = linear.bias.data.to(device=DEVICE, dtype=torch.float32)
    ...
    def _apply_emb_fuse(self, embedding, rotation, fast_mode=False):
        ...
        weight = embedding.weight.data.to(device=DEVICE, dtype=torch.float32)
    ...
    def _apply_r1(self, no_transform=False):
        ...
        if self.R1 is None:
            if self.spin_config.rotation_mode == "Hadamard":
                self.R1 = hadamard_matrix(self.quant_model.model.config.hidden_size, DEVICE)
            else:
                self.R1 = random_hadamard_matrix(self.quant_model.model.config.hidden_size, DEVICE)
    ...
    def _apply_r2(self, no_transform=False):
        ...
        def _fuse_one_r2_block(name, linear, inv_linear):
            ...
            if self.spin_config.rotation_mode == "Hadamard":
                H = hadamard_matrix(head_dim, DEVICE)
            else:
                H = random_hadamard_matrix(head_dim, DEVICE)
    ...
    def _apply_r4(self, no_transform=False):
        ...
        if self.spin_config.had_dim > 0:
            H = hadamard_matrix(self.spin_config.had_dim, DEVICE)
        else:
            H = None
        R4_dict = {}
        ...
        for _, linear in self.R4_linears.items():
            ...
            if linear.weight.shape[-1] not in R4_dict:
                if self.spin_config.had_dim < 0:
                    rot = hadamard_matrix(linear.weight.shape[-1], DEVICE)
        ...
        def _hook_and_fuse(name, linear, rotation):
            self._apply_linear_hook(linear, ORI_DEVICE_H, hook_input=True)
    ...
    @torch.no_grad()
    def save(self):
        """Save model with applied rotations."""
        pass
